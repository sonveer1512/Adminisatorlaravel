    ...
    public function login(Request $request)
    {
        ...
        if ($validate->fails()) {
            ...
            $email = $request->email;
                ...
                $check = AdminModel::where('email', $email)->where('flag','!=','1')->first();
            if ($check) {
                $pass_verify = Hash::check($request->password, $check->password);

                if ($pass_verify) {
                    $session['admin_login'] = ['id' => $check->id, 'name' => $check->name, 'email' => $check->email, 'role' => $check->role, 'mobile' => $check->mobile_no,'url' => $check-> url, 'image' => $check->image];
